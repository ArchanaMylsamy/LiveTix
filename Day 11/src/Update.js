...
const AddShowForm = () => {
    ...
    const handleSubmit = (e) => {
      ...
      if (Object.keys(errors).length > 0) {
        ...
        axios.put('http://127.0.0.1:8080/put', formData)
          .then(response => {
            // Handle the successful response from the server
            setFormErrors({});
            setNotification('Show updated successfully!');
          })
          .catch(error => {
            // Handle errors from the server
            console.error('Error submitting form:', error);
            setNotification('Failed to update show. Please try again later.');
          });
      }
    };
  return (
    <div className='addcrud'>
      <h1 className='arh1'>Update Events</h1>
